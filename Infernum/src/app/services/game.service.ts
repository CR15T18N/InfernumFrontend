...
export class GameService {
  ...
  private mapGame(bg: BackendGame): Game {
    ...
    return {
      ...
      requirements: bg.requirements ?? null,
    };
  }
  ...
  async getLibrary(): Promise<Game[]> {
    try {
      const res = await firstValueFrom(
        this.http.get<{ status: string; games?: any[]; message?: string }>(
          `${this.apiUrl}/library/games`,
        ),
      );
